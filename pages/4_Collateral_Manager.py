...
with tab3:
    ...
    # ── Haircut: compute default from current selections, show as editable ──
    # Key includes ac + symbol + sub_type so it resets when any of those change
    hc_key     = f"col_hc_{new_ac}_{new_symbol}_{new_sub_type}"
    default_hc = get_default_haircut(new_ac, new_sub_type, new_symbol)

    # If the key changes (asset class / symbol / sub_type changed), reset to default
